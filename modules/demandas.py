...
def render():
    ...
    # Inicializa chaves no session_state se não existirem
    ...
    def reset_form():
        # Limpa textos
        chaves_texto = ["nome_text", "prot_text", "prot_demanda_text", "cto_text", "sinal_text", "coords_text"]
        ...
        # Limpa portas se houver
        for i in range(1, 17):
            if f"p_{i}" in st.session_state:
                st.session_state[f"p_{i}"] = False
        if "p_todos" in st.session_state:
            st.session_state["p_todos"] = False
    ...
    with st.container():
        ...
        with col_top2:
            # O truque está no 'value='
            protocolo_demanda = st.text_input("Protocolo da Demanda", key="prot_demanda_text", value=st.session_state.get("prot_demanda_text", ""))

    st.divider()

    col_a, col_b = st.columns(2)
    with col_a:
        nome_cliente = st.text_input("Nome do Cliente", key="nome_text", value=st.session_state.get("nome_text", ""))
        protocolo = st.text_input("Protocolo da Solicitação", key="prot_text", value=st.session_state.get("prot_text", ""))
    with col_b:
    ...
    with col1:
        num_cto = st.text_input("Número da CTO", key="cto_text", value=st.session_state.get("cto_text", ""))
        sinal_cto = st.text_input("Sinal da CTO (Power Meter)", key="sinal_text", value=st.session_state.get("sinal_text", ""))
    ...
    with col2:
        coords = st.text_input("Coordenadas (Lat, Long)", key="coords_text", value=st.session_state.get("coords_text", ""))
    ...
    with c_salvar:
        if st.button("💾 Salvar na Planilha", type="primary", use_container_width=True):
            if tecnico_selecionado == " " or not nome_cliente:
                ...
                if aba:
                    try:
                        prob_f = problema + (f" (Portas: {', '.join(portas_selecionadas)})" if portas_selecionadas else "")
