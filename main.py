...
st.set_page_config(page_title="Sistema Atendimento - Portfolio Demo", layout="wide", page_icon="📊")

# --- AVISO GLOBAL DE DEMONSTRAÇÃO ---
st.warning("⚠️ **VERSÃO DE PORTFÓLIO (DEMONSTRAÇÃO):** As integrações com APIs, banco de dados e automações reais foram desativadas. Os dados apresentados são fictícios para fins de demonstração.")
...
try:
    ...
    pass # Ignora caso o styles.py não esteja perfeitamente configurado na nuvem
...
try:
    ...
    st.warning(f"Aviso: Alguns módulos estão sendo carregados em modo de segurança... (Erro local ocultado na demo)")

fuso_br = pytz.timezone('America/Sao_Paulo')
agora = datetime.now(fuso_br)

# --- 5. GATILHO AUTOMÁTICO (MODO DEMO) ---
HORA_ALVO = 20
MIN_ALVO = 36
...
if agora.hour == HORA_ALVO and agora.minute == MIN_ALVO:
    if st.session_state.dia_disparo != agora.day:
        ...
        with st.status("🤖 [DEMO] Simulando envio de relatório diário...") as status:
            try:
                # Simulação do processo sem chamar a função real
                import time
                time.sleep(2)
                status.update(label="✅ [DEMO] Relatório simulado com sucesso! (Nenhum dado real foi enviado)", state="complete")
            except:
                status.update(label="❌ Módulo indisponível.", state="error")

# --- 6. INTERFACE ---
st.sidebar.title("🚀 Menu Principal")
st.sidebar.info("💡 **Dica do Portfólio:** Navegue pelos menus abaixo para ver a interface e as lógicas de negócio em funcionamento.")
...
with st.sidebar:
        ...
        st.divider()
        st.caption("Desenvolvido por João Vitor")
...
try:
    ...
    st.error("O módulo selecionado está em manutenção para a versão de demonstração.")
