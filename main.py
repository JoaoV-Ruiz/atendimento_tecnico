...
if agora.hour == HORA_ALVO and agora.minute == MIN_ALVO:
    if st.session_state.dia_disparo != agora.day:
        ...
        # Log interno para você ver no console do Streamlit Cloud
        print(f"🚀 Iniciando disparo automático: {agora.strftime('%H:%M:%S')}")
        
        with st.status("🤖 Processando relatório das 23:45...") as status:
            # Chama a função no amarelos.py
            ...
            if sucesso:
                ...
                # Se falhar, permite tentar de novo no próximo minuto/refresh
